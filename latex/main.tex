...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.62\textwidth]{matchup_correlation.png}
...
Figure~\ref{fig:gmm-profiles} shows spider (radar) plots of empirical centroid profiles for each hard \(K = 5\) assignment---the sample mean of the seven standardized style features among fighters with that cluster ID under maximum-posterior labeling from the fitted mixture (assignments and posteriors archived in \texttt{ufc\_gmm\_comparison.csv}).  One panel per cluster ID, shared angular axes; values along each spoke are averages in the same standardized units used to fit the mixture (zero is the mean across qualified fighters), with all panels plotted on the same radial scale.  The five clusters are interpretable:
...
\begin{figure}[!htbp]
\centering
\includegraphics[height=0.50\textheight,keepaspectratio]{gmm_profiles_k5.png}
\caption{Spider (radar) plots of empirical centroid profiles by hard \(K = 5\) cluster assignment (maximum posterior under the fitted mixture; same cluster IDs as \texttt{ufc\_gmm\_comparison.csv}, the heatmap, and Table~\ref{tab:champion-cluster-map}): one panel per cluster, common radial scale, ring labels read relative to the qualified-fighter mean (zero is average); submission-leaning grappler (0), active distance striker (1), low-activity (2), baseline generalist (3), takedown-heavy wrestler (4).  Source: notebook~10.}
...
\begin{figure}[H]
\centering
\includegraphics[height=0.55\textheight,keepaspectratio]{ae_champions.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[height=0.42\textheight,keepaspectratio]{style_heatmap.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[height=0.32\textheight,keepaspectratio]{hybrid_winrate.png}
